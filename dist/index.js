...
function energy(config) {
    ...
    return [
        resolveEnergyPlugin(pluginConfig),
        ...resolveFullReloadConfig(pluginConfig)
    ];
}
function resolveEnergyPlugin(pluginConfig) {
    ...
    return {
        ...
        config: (config, { command, mode }) => {
            ...
            return {
                ...
                server: {
                    origin: userConfig.server?.origin ?? "__energy_vite_placeholder__",
                    ...process.env.ENERGY_SAIL ? {
        ...
        transform(code) {
            if (resolvedConfig.command === "serve") {
                code = code.replace(/__energy_vite_placeholder__/g, viteDevServerUrl);
        ...
        configureServer(server) {
            ...
            server.httpServer?.once("listening", () => {
                ...
                if (isAddressInfo(address)) {
                    ...
                    setTimeout(() => {
                        server.config.logger.info(`${colors.green(`${colors.bold("ENERGY")} ${energyVersion()}`)}  ${colors.dim("plugin")} ${colors.bold(`v${pluginVersion()}`)}`);
...
function ensureCommandShouldRunInEnvironment(command, env) {
    if (command === "build" || env.ENERGY_BYPASS_ENV_CHECK === "1") {
        return;
    }
    if (typeof env.ENERGY_VAPOR !== "undefined") {
        throw Error("You should not run the Vite HMR server on Vapor. You should build your assets for production instead. To disable this ENV check you may set ENERGY_BYPASS_ENV_CHECK=1");
    }
    if (typeof env.ENERGY_FORGE !== "undefined") {
        throw Error("You should not run the Vite HMR server in your Forge deployment script. You should build your assets for production instead. To disable this ENV check you may set ENERGY_BYPASS_ENV_CHECK=1");
    }
    if (typeof env.ENERGY_ENVOYER !== "undefined") {
        throw Error("You should not run the Vite HMR server in your Envoyer hook. You should build your assets for production instead. To disable this ENV check you may set ENERGY_BYPASS_ENV_CHECK=1");
    }
    if (typeof env.CI !== "undefined") {
        throw Error("You should not run the Vite HMR server in CI environments. You should build your assets for production instead. To disable this ENV check you may set ENERGY_BYPASS_ENV_CHECK=1");
    }
}
function energyVersion() {
    try {
        const composer = JSON.parse(fs.readFileSync("composer.lock").toString());
        return composer.packages?.find((composerPackage) => composerPackage.name === "priveted/energy-framework")?.version ?? "";
    } catch {
        return "";
    }
}
...
function resolvePluginConfig(config) {
    if (typeof config === "undefined") {
        throw new Error("energy-vite-plugin: missing configuration.");
    }
    if (typeof config === "string" || Array.isArray(config)) {
        config = { input: config, ssr: config };
    }
    if (typeof config.input === "undefined") {
        throw new Error('energy-vite-plugin: missing configuration for "input".');
    }
    if (typeof config.publicDirectory === "string") {
        config.publicDirectory = config.publicDirectory.trim().replace(/^\/+/, "");
        if (config.publicDirectory === "") {
            throw new Error("energy-vite-plugin: publicDirectory must be a subdirectory. E.g. 'public'.");
        }
    }
    if (typeof config.buildDirectory === "string") {
        config.buildDirectory = config.buildDirectory.trim().replace(/^\/+/, "").replace(/\/+$/, "");
        if (config.buildDirectory === "") {
            throw new Error("energy-vite-plugin: buildDirectory must be a subdirectory. E.g. 'build'.");
        }
    }
...
function resolveFullReloadConfig({ refresh: config }) {
    ...
    return config.flatMap((c) => {
        ...
        plugin.__energy_plugin_config = c;
        return plugin;
    });
}
